...
namespace aMuleCtrl.Database
{
    class Magnet
    {
        public static SQLiteConnection Initialize()
        {
            ...
            string sql = "CREATE TABLE IF NOT EXISTS magnets(id integer NOT NULL PRIMARY KEY AUTOINCREMENT, "  + 
            ...
            using (SQLiteConnection conn = Initialize())
            {
                ...
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        sql = "INSERT INTO magnets (link, designation_id) VALUES ('" + HttpUtility.HtmlEncode(link) + "', " + designation_id + ")";
        ...
        public static void RemoveLink(String link)
        {
            using (SQLiteConnection conn = Initialize())
            {
                String sql = "DELETE * FROM magnets WHERE link = '" + HttpUtility.HtmlEncode(link) + "'";
                SQLiteCommand cmd = new SQLiteCommand(sql, conn);
                cmd.ExecuteNonQuery();
            }
        }
        ...
        public static List<String> GetAllLinks()
        {
            ...
            String sql = "SELECT * FROM magnets";
            SQLiteCommand cmd = new SQLiteCommand(sql, conn);
        ...
        public static List<String> GetLinksByDesignation(String designation)
        {
            ...
            using (SQLiteConnection conn = Initialize())
            {
                ...
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    {
                        int designation_id = reader.GetInt32(0);
                        sql = "SELECT * FROM magnets WHERE designation_id = " + designation_id;
                        cmd = new SQLiteCommand(sql, conn);
                        using (SQLiteDataReader linksReader = cmd.ExecuteReader())
                        {
                            while (linksReader.Read())
        ...
        {
            List<String> ret = new List<string>();
            String sql = "SELECT * FROM magnets WHERE link = '" + HttpUtility.HtmlEncode(link) + "'";
            ...
            using (SQLiteConnection conn = Initialize())
            {
                ...
                using (SQLiteDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        cmd = new SQLiteCommand("UPDATE magnets SET download_times = " + times + " WHERE id = " + id, conn);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
